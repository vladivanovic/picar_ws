...
Bring up the AgenticROS Picar 4WD in Gazebo Harmonic with full ROS-side
plumbing:
  * gz sim agenticros_indoor.sdf                  (the world + everything in it)
  * ros_gz_sim create                             (spawn the Picar if it isn't
...
Launch args:
  ...
  use_rviz    (default 'false')                    Show RViz with the Picar view config.
  use_sim_time (default 'true')                    /clock is bridged; downstream
                                                  nodes should honour this.
  x, y, z, yaw                                    Spawn pose for the Picar
  bridge_config_file                              Override the bridge YAML.
  gui         (default 'true')                    Headless if 'false' (CI / docker).
  rviz_config                                     Override the RViz config path.

Examples:
  ros2 launch agenticros_sim sim_picar.launch.py
  ros2 launch agenticros_sim sim_picar.launch.py use_rviz:=true
  ros2 launch agenticros_sim sim_picar.launch.py gui:=false       # headless
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    default_bridge = os.path.join(pkg_share, "config", "picar_bridge.yaml")
    default_rviz = os.path.join(pkg_share, "config", "picar_view.rviz")
    urdf_xacro = os.path.join(pkg_share, "urdf", "picar_robot.urdf.xacre") # Typo in my thought, let's check actual file
    # Wait, I copied it as picar_robot.urdf.xacro
    urdf_xacro = os.path.join(pkg_share, "urdf", "picar_robot.urdf.xacro")
    ...
    use_rviz_arg = DeclareLaunchArgument(
        ...
        description="Open RViz with the Picar view config.",
    )
    use_sim_time_arg = DeclareLaunchArgument(
        "use_sim_time", default_value="true",
        description="Tell ROS nodes to use /clock published by gz.",
    )
    x_arg = DeclareLaunchArgument("x", default_value="0.0", description="Picar spawn X (m)")
    y_arg = DeclareLaunchArgument("y", default_value="0.0", description="Picar spawn Y (m)")
    z_arg = DeclareLaunchArgument("z", default_value="0.1", description="Picar spawn Z (m)")
    yaw_arg = DeclareLaunchArgument("yaw", default_value="0.0", description="Picar yaw (rad)")
    ...
    # ---------- Spawn the Picar ----------
    # We use -topic robot_description because we want to spawn the URDF/Xacro 
    # geometry provided by robot_state_publisher, not an SDF file.
    spawn_picar = Node(
        package="ros_gz_sim",
        executable="create",
        name="spawn_picar",
    ...
    # ---------- robot_state_publisher (URDF mirror) ----------
